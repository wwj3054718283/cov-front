...
<script>
import shanxizizhiqu from "echarts/map/json/province/shanxi";
export default {
    name: "shanxizizhiqu",
    methods: {
        async getData() {
            this.$echarts.registerMap("shanxizizhiqu", shanxizizhiqu);
            const myTable = this.$echarts.init(this.$refs.dataTable);
            const res = await this.$http.get("shanxizizhiqu");
        ...
        async getArea() {
            const myChart = this.$echarts.init(this.$refs.shanxizizhiqu);
            // 注册矢量地图数据
            this.$echarts.registerMap("shanxizizhiqu", shanxizizhiqu);
            const res = await this.$http.get("shanxizizhiqu");
